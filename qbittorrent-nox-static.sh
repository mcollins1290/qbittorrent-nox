...
#######################################################################################################################################################
unset qb_skip_delete qb_git_proxy qb_curl_proxy qb_install_dir qb_build_dir qb_working_dir qb_modules_test qb_python_version KERNEL
#######################################################################################################################################################
# Define color values.
#######################################################################################################################################################
...
set_default_values() {
        ...
        #
        qb_modules=("all" "linux_headers" "qtbase_host" "qttools_host" "glibc" "zlib" "iconv" "icu" "openssl" "boost" "libtorrent" "double_conversion" "qtbase_pi" "qttools_pi" "qbittorrent")
        #
        delete=()
        #
        qb_required_pkgs=("build-essential" "bison" "curl" "pkg-config" "automake" "libtool" "git" "perl" "python${qb_python_version}" "python${qb_python_version}-dev" "python${qb_python_version}-numpy" "rsync" "gawk" "texinfo" "gettext" "cmake" "ninja-build")
        #
...
custom_flags_set() {
        ...
        CFLAGS="-O2 -march=armv8-a+crc -mcpu=cortex-a72 -mtune=cortex-a72"
        CXXFLAGS="-std=${cxx_standard} --static -static -w ${CFLAGS} -I${include_dir}"
        CPPFLAGS="--static -static -w ${CFLAGS} -I${include_dir}"
...
set_module_urls() {
        ...
        icu_url="https://github.com/unicode-org/icu/archive/refs/tags/release-${icu_github_tag}.tar.gz"
        #
        ...
        boost_url_status="$(curl_curl -so /dev/null --head --write-out '%{http_code}' https://archives.boost.io/release/${boost_version}/source/boost_${boost_version//./_}.tar.gz)"
	...
	qtbase_host_url="https://download.qt.io/archive/qt/${qt_version_short}/${qtbase_github_tag/v/}/submodules/qtbase-everywhere-src-${qtbase_github_tag/v/}.tar.xz"
        qtbase_pi_url="https://download.qt.io/archive/qt/${qt_version_short}/${qtbase_github_tag/v/}/submodules/qtbase-everywhere-src-${qtbase_github_tag/v/}.tar.xz"
        qttools_host_url="https://download.qt.io/archive/qt/${qt_version_short}/${qttools_github_tag/v/}/submodules/qttools-everywhere-src-${qttools_github_tag/v/}.tar.xz"
	qttools_pi_url="https://download.qt.io/archive/qt/${qt_version_short}/${qttools_github_tag/v/}/submodules/qttools-everywhere-src-${qttools_github_tag/v/}.tar.xz"
        #
...
installation_modules "${@}"
#######################################################################################################################################################
# qtbase_host installation
#######################################################################################################################################################
application_name qtbase_host
...
if [[ "${!app_name_skip:-yes}" == 'no' ]] || [[ "${1}" == "${app_name}" ]]; then
		...
		if [[ ! -d "${qb_install_dir}/qt-host" ]]; then
			...
			post_build
			#
			cmake --install build-host |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
		fi
...
#######################################################################################################################################################
# qttools_host installation
#######################################################################################################################################################
application_name qttools_host
#
if [[ "${!app_name_skip:-yes}" == 'no' ]] || [[ "${1}" == "${app_name}" ]]; then
        if [[ ! -d "${qb_install_dir}/qt-host" ]]; then
                echo -e "${tn} ${urc}${clr} Warning${cend} This module depends on the qtbase_host module. Use them together: ${clm}qtbase_host qttools${cend}"
        else
                #
                download_file "${app_name}" "${!app_url}"
                #
...
if [[ "${!app_name_skip:-yes}" = 'no' || "${1}" = "${app_name}" ]]; then
        ...
        #
        KERNEL=kernel8 # Raspberry Pi 4 (64-Bit) - see https://www.raspberrypi.org/documentation/linux/kernel/building.md
        make ARCH=arm64 INSTALL_HDR_PATH="${qb_install_dir}" headers_install |& tee "${qb_install_dir}/logs/${app_name}.log.txt"
        unset KERNEL
        #
        post_build
        #
...
if [[ "${!app_name_skip:-yes}" == 'no' || "${1}" == "${app_name}" ]]; then
        ...
        "${qb_install_dir}/${app_name}-release-${icu_github_tag}/icu4c/source/runConfigureICU" Linux/gcc
        make -j"$(nproc)"
        _cd "${qb_install_dir}/${app_name}-release-${icu_github_tag}/icu4c/source"
        #
        custom_flags_set
...
if [[ "${!app_name_skip:-yes}" = 'no' || "${1}" = "${app_name}" ]]; then
        ...
        download_file "${app_name}" "${!app_url}"
        #
        ./Configure linux-aarch64 --prefix="${qb_install_dir}" --openssldir="/etc/ssl" threads no-shared no-dso no-comp CXXFLAGS="${CXXFLAGS}" CPPFLAGS="${CPPFLAGS}" LDFLAGS="${LDFLAGS}" |& tee "${qb_install_dir}/logs/${app_name}.log.txt"
        make -j"$(nproc)" |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
        #
        post_build
        #
...
if [[ "${!app_name_skip:-yes}" = 'no' ]] || [[ "${1}" = "${app_name}" ]]; then
	...
	# ------------------------------------------------------------------
	# Build Boost for Raspberry Pi (AArch64) using cross toolchain
	# ------------------------------------------------------------------

	BOOST_RPI_PREFIX="${qb_install_dir}/boost-rpi"

	# Clean old Pi Boost install, if any
	rm -rf "${BOOST_RPI_PREFIX}"

	_cd "${qb_install_dir}/boost"

	# Make sure our cross toolchain is on PATH
	TARGET=aarch64-linux-gnu

	# Tell Boost about the AArch64 compiler
	cat >> project-config.jam <<EOF

using gcc : aarch64 : ${TARGET}-g++ :
  <cxxflags>-O2 -pipe</cxxflags>
  <linkflags>-O2 -pipe</linkflags>
  ;
EOF
	# Build and install only the libs qBittorrent/libtorrent typically need
	./b2 \
	  toolset=gcc-aarch64 \
	  address-model=64 \
	  architecture=arm \
	  binary-format=elf \
	  --prefix="${BOOST_RPI_PREFIX}" \
	  --layout=tagged \
	  ...
	  --with-regex \
	  --with-date_time \
	  install |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
        #
        if [[ "${boost_url_status}" =~ (403|404) ]]; then
...
if [[ "${!app_name_skip:-yes}" == 'no' ]] || [[ "${1}" == "${app_name}" ]]; then
        if [[ ! -d "${qb_install_dir}/boost" ]]; then
                ...
                download_folder "${app_name}" "${!app_github_url}"
                #
                BOOST_ROOT="${qb_install_dir}/boost-rpi"
                BOOST_INCLUDEDIR="${qb_install_dir}/boost-rpi"
                BOOST_BUILD_PATH="${qb_install_dir}/boost-rpi"
                #
                cmake -Wno-dev -Wno-deprecated -G Ninja -B build \
                        ...
                        -D CMAKE_BUILD_TYPE="Release" \
                        -D CMAKE_CXX_STANDARD="${standard}" \
                        -D CMAKE_PREFIX_PATH="${qb_install_dir};${qb_install_dir}/boost-rpi" \
...
#######################################################################################################################################################
# qtbase_pi installation
#######################################################################################################################################################
application_name qtbase_pi

if [[ "${!app_name_skip:-yes}" == 'no' ]] || [[ "${1}" == "${app_name}" ]]; then
        #
        download_file "${app_name}" "${!app_url}"
        #
        read -p "Now remove locale warning messages from qtbase/src/corelib/kernel/qcoreapplication.cpp and then press enter to continue"
        #
        ...
        custom_flags_set
        #
	./configure -static -openssl-linked -- -Wno-dev -Wno-deprecated -G Ninja -B build-rpi \
		-D CMAKE_BUILD_TYPE=Release \
		-D CMAKE_VERBOSE_MAKEFILE=OFF \
		-D CMAKE_CXX_STANDARD="${standard}" \
		...
		-D QT_QMAKE_TARGET_MKSPEC=devices/linux-rasp-pi4-v3d-g++ |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
        #
        cmake --build build-rpi |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
        #
        post_build
        #
        cmake --install build-rpi |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
        #
        _fix_static_links "${app_name}"
        #
        delete_function "${app_name}"
...
#######################################################################################################################################################
# qttools_pi installation
#######################################################################################################################################################
application_name qttools_pi
#
if [[ "${!app_name_skip:-yes}" == 'no' ]] || [[ "${1}" == "${app_name}" ]]; then
        if [[ ! -d "${qb_install_dir}/qt-rpi" ]]; then
                echo -e "${tn} ${urc}${clr} Warning${cend} This module depends on the qtbase_pi module. Use them together: ${clm}qtbase_pi qttools_pi${cend}"
        else
		#
		download_file "${app_name}" "${!app_url}"
		#
		...
		"${qb_install_dir}/qt-rpi/bin/qt-cmake" -Wno-dev -Wno-deprecated -G Ninja -B build-rpi \
		-D CMAKE_VERBOSE_MAKEFILE=OFF \
		-D CMAKE_BUILD_TYPE=Release \
		-D CMAKE_CXX_STANDARD="${standard}" \
		-D CMAKE_INSTALL_PREFIX="${qb_install_dir}/qt-rpi" \
		...
		-D CMAKE_SKIP_INSTALL_RPATH=ON \
		-D QT_BUILD_EXAMPLES=OFF \
		-D QT_BUILD_TESTS=OFF |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
		#
		cmake --build build-rpi |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
		#
		post_build
...
if [[ "${!app_name_skip:-yes}" == 'no' ]] || [[ "${1}" == "${app_name}" ]]; then
		...
		-D GUI=OFF \
		\
		-D CMAKE_INSTALL_PREFIX="${qb_install_dir}/qbittorrent-rpi" \
		-D QBT_STAGING_ROOT="${qb_install_dir}" \
		\
		-D CMAKE_PREFIX_PATH="${qb_install_dir}/qt-rpi;${qb_install_dir}" \
		-D Boost_ROOT="${qb_install_dir}/boost-rpi" \
		-D CMAKE_CXX_FLAGS="${CXXFLAGS}" |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
                cmake --build build-rpi-qbt |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
                #
                post_build
                #
                cmake --install build-rpi-qbt |& tee -a "${qb_install_dir}/logs/${app_name}.log.txt"
                #
                [[ -f "${qb_install_dir}/qbittorrent-rpi/bin/qbittorrent-nox" ]] && cp -f "${qb_install_dir}/qbittorrent-rpi/bin/qbittorrent-nox" "${qb_install_dir}/completed/qbittorrent-nox"
